...
{
    require_once('../config/config.php');
    $ejecutar = $mysqli->prepare("UPDATE  servicios_a_realizar_trabajadore  SET fecha_inicio = ?, hora_inicio = ? WHERE id_servicioarealizar = ?");
...
{
    require_once('../config/config.php');
    $ejecutar = $mysqli->prepare("UPDATE  servicios_a_realizar_trabajadore  SET fecha_fin = ?, hora_fin = ? WHERE id_servicioarealizar = ?");
    $ejecutar->bind_param("sss", $json->fecha,$json->hora,$json->numeroservicio);
 $ejecutar->execute();
if ($ejecutar->affected_rows){
    $respuesta = "Exito";
